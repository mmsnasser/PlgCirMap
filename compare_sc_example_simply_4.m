% compare_sc_example_simply_4.m
% Nasser, September 3, 2019
% In this code, we consider:
% bounded simply connected domain G inside polygon with 4 vertices
% 
...
p=polygon(ver{1}); 
tic
fsc = center(diskmap(p,options),alpha);% fsc=f^-1 is the invers map from 
                        % the circular domain D onto the domain G compute 
                        % by SC toolbox such that 
                        % fsc(0)=alpha and fsc(1)=ver{1}(end)
toc
%%
